...
contract MegaFactory is Ownable {
    ...
    modifier onlyAuthorizedDeployer(string memory id) {
        require(
            completedDeployments[id].vaultType == VaultType.None,
            "cannot reuse id"
        );
    ...
    constructor(address _storage, address _multisig) Ownable(msg.sender) {
        multisig = _multisig;
    ...
    function createRegularVault(
        string calldata id,
        address underlying
    ) external onlyAuthorizedDeployer(id) {
        address vault = IVaultFactory(
            vaultFactories[uint256(VaultType.Regular)]
        ).deploy(actualStorage, underlying);
    ...
    function createRegularVaultUsingUpgradableStrategy(
        ...
        address strategy = IStrategyFactory(
            strategyFactories[uint256(StrategyType.Upgradable)]
        ).deploy(actualStorage, vault, strategyImplementation);
    ...
    function createUniV3Vault(
        string calldata id,
        uint256 uniV3PoolId
    ) external onlyAuthorizedDeployer(id) {
        address vault = IUniV3VaultFactory(
            vaultFactories[uint256(VaultType.UniV3)]
        ).deploy(actualStorage, uniV3PoolId);
